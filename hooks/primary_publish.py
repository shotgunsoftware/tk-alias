...
class PrimaryPublishHook(Hook):
    ...
    def execute(
        ...
        # Save the actual file, after 60 seconds just go with the unsaved file
        self.parent.engine.send_and_wait(commands.FileSaveCommand(""))
        ...
        publish_data = self._register_publish(publish_path,
                                              publish_name,
                                              sg_task,
                                              fields["version"],
                                              output["tank_type"],
        ...
        version = self.create_version_entity(publish_data, sg_task, thumbnail_path)

        sg_task["primary_created_version"] = version
    ...
    def create_version_entity(self, publish_data, task_data, thumbnail_path):
        """
        ...
        data = {
            ...
            "user": user,
            "published_files": [
                { "type": "PublishedFile", "id": publish_id }
            ]
        }

        if task_data:
            data["sg_task"] = { "type": "Task", "id": task_data["id"] }

        entity = self.parent.context.entity
        if entity and entity["type"] == "Asset":
            data["entity"] = { "type": "Asset", "id": entity["id"] }
