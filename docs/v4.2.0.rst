...
.. note::
    The hook method ``get_alias_api_extensions_path`` returns the path to the directory containing the hook file; this means that the ``my_alias_api_extensions_functions.py`` file must be placed in the same directory as the hook file.
