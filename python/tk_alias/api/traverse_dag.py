...
def node_has_non_zero_transform(node):
    ...
    if not api_utils.is_success(status):
        raise alias_api.AliasPythonExcept("Failed to retrieve node global transformation matrix.")
...
def node_has_unused_curve_on_surface(node, check_exists=False, delete=False):
    ...
    return api_utils.success_status() if has_unused_curves else api_utils.failure_status()
