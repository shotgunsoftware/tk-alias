...
class AliasPyUtils(AliasPyBase):
    ...
    def success_status(self, int_value=False):
        ...
        status = self.alias_py.AlStatusCode.Success
        if int_value:
            return status.value
        return status

    def failure_status(self, int_value=False):
        """
        ...
        status = self.alias_py.AlStatusCode.Failure
        if int_value:
            return status.value
        return status

    def is_success(self, alias_status):
        """
        ...
        success = self.alias_py.AlStatusCode.Success

        if isinstance(alias_status, self.alias_py.AlStatusCode):
            return alias_status == success

        return alias_status == success.value

    def raise_exception(self, msg, error_status=None):
        """
    ...
    def camera_node_types(self):
        ...
        return [
            self.alias_py.AlObjectType.CameraEyeType,
            self.alias_py.AlObjectType.CameraViewType,
            self.alias_py.AlObjectType.CameraUpType,
        ]

    def light_node_types(self):
        """
        ...
        return [
            self.alias_py.AlObjectType.LightNodeType,
            self.alias_py.AlObjectType.LightLookAtNodeType,
            self.alias_py.AlObjectType.LightUpNodeType,
        ]
