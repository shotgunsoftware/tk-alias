...
class AliasPyDagNode(AliasPyBase):
    ...
    def is_node_template(self, node):
        """
    ...
    def get_instanced_nodes(self, nodes=None, check_exists=False):
        """
        ...
        if nodes:
            for node in nodes:
                ...
                if self.alias_py.py_utils.is_success(
                    self.alias_py.py_traverse_dag.node_is_instance(node)
                ):
                    if check_exists:
                        return True
    ...
    def get_nodes_with_construction_history(
        self, nodes=None, check_exists=False, skip_node_types=None
    ):
        """
        ...
        if nodes:
            for node in nodes:
                ...
                if self.alias_py.has_history(node):
                    if check_exists:
                        return True
            ...
            # NOTE check_exists does not apply to traverse dag yet
            input_data = self.alias_py.TraverseDagInputData(skip_node_types, False)
    ...
    def get_nodes_with_non_zero_transform(
        ...
        if nodes:
            for node in nodes:
                ...
                status = self.alias_py.py_traverse_dag.node_has_non_zero_transform(node)
                if self.alias_py.py_utils.is_success(status):
                    if check_exists:
                        return True
            ...
            # NOTE check_exists does not apply to traverse dag yet
            input_data = self.alias_py.TraverseDagInputData(skip_node_types, False)
            result = self.alias_py.search_node_has_non_zero_transform(input_data)
            return result.nodes
    ...
    def get_nodes_with_non_origin_pivot(
        ...
        if nodes:
            for node in nodes:
                ...
                status = self.alias_py.py_traverse_dag.node_has_non_origin_pivot(node)
                if self.alias_py.py_utils.is_success(status):
                    if check_exists:
                        return True
                    invalid_nodes.append(node)
    ...
    def get_nodes_with_unused_curves_on_surface(self, nodes=None, check_exists=False):
        """
    ...
    def get_unused_curves_on_surface_for_nodes(self, nodes=None):
        """
        ...
        if nodes:
            for node in nodes:
                ...
                if (
                    not node
                    or node.type() != self.alias_py.AlObjectType.SurfaceNodeType
                ):
                    continue
